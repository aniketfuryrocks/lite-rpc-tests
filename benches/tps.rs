...
async fn foo(lite_client: Arc<LiteClient>, bench_time: Duration) -> Metric {
        ...
        tokio::spawn(async move {
            ...
            while start_time.elapsed() < bench_time {
                let tx = create_transaction(
                    &funded_payer,
                    *latest_block_hash.get_latest_blockhash().await.unwrap(),
                );

                if lite_client.send_transaction(&tx).await.is_ok() {
                    tx_send.send(tx.get_signature().to_string()).unwrap();
                    tx_sent += 1;
                }
            }
    ...
    let confirm_fut = tokio::spawn(async move {
        ...
        while let Some(sig) = tx_recv.recv().await {
            ...
            println!("recv {sig} {} con {}", metric.txs_sent, metric.txs_confirmed);
